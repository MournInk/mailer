    ...
    >
      {selected ? (
        ...
        <div className="empty-state fade-up">
          ...
          <p className="empty-title">选择一封邮件</p>
  ...
  return (
    <>
      <header className="mv-bar">
        ...
        <button
          className={`icon-btn${msg.starred ? " active" : ""}`}
        ...
        <button className="btn btn-ghost btn-sm" onClick={reply} title="回复发件人">
      ...
      <div className="mv-scroll">
        <article className="mv-doc fade-up">
          ...
          {analysis && (
            <section className="card mv-ai" aria-label="AI 分析">
              ...
              {analysis.reason && (
                <>
                  ...
                  {showReason && <p className="mv-reason">{analysis.reason}</p>}
                </>
              )}
            </section>
          )}
          ...
          {body && body.blocked > 0 && !showImages && (
            <div className="mv-imgbar">
              ...
              <span className="mv-imgbar-text">
                已阻止 {body.blocked} 处远程图片，避免发件人追踪你的阅读行为。
              </span>
          ...
          {msg.attachments.length > 0 && (
            <section className="mv-atts" aria-label="附件">
              <div className="mv-atts-head">附件 {msg.attachments.length} 个</div>
...
function DeleteControl({ onDelete }: { onDelete: (onServer: boolean) => void }) {
  ...
  return (
    <div className="mv-del" ref={wrapRef}>
      <button
        className="btn btn-ghost btn-sm mv-del-main"
      ...
      <button
        className="btn btn-ghost btn-sm mv-del-caret"
        onClick={() => setOpen((v) => !v)}
        aria-haspopup="menu"
        aria-expanded={open}
        aria-label="删除选项"
...
function AttachmentRow({ att }: { att: AttachmentMeta }) {
  return (
    <div className="mv-att" title={att.mime}>
      <Icon name="paperclip" size={15} className="mv-att-icon" />
